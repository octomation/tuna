...
- [ ] Run `go get github.com/google/uuid` to add dependency
- [ ] Run `go build` to ensure no compilation errors
- [ ] Run `go test ./internal/assistant/...` to verify assistant package tests
- [ ] Run `go test ./internal/plan/...` to verify plan package tests
- [ ] Create test assistant with `tuna init test-assistant`
- [ ] Run `tuna plan test-assistant` and verify output
- [ ] Verify `plan.toml` structure matches specification
- [ ] Test with multiple models: `tuna plan test-assistant -m "gpt-4,claude-3"`
- [ ] Test error cases: missing directory, empty system prompt
