---
id: 68
database_id: 3703773550
node_id: I_kwDOPyI7hs7cwxlu
status: closed
title: "feature: implement `tuna view` command"
labels: ["type: feature","scope: code","impact: high","effort: hard"]
url: https://github.com/octomation/tuna/issues/68
created_at: 2025-12-07T17:55:05Z
updated_at: 2025-12-07T19:58:44Z
---

# feature: implement `tuna view` command
...
| `PgUp`/`PgDn` | Scroll half page in focused column               |
...
All model responses are displayed side-by-side in columns for easy comparison. The focused column is highlighted with a distinct border.

**Column layout rules:**
- Columns fill all available horizontal space
- Maximum 2 columns visible at once
- Minimum 1 column
- If 1 model → single column takes full width
- If 2 models → 2 columns, each taking half the width
- If 3+ models → 2 columns visible + horizontal scrolling

**Input query section:**
- By default, shows 2 lines of the input query (collapsed)
- Press `Tab` to expand and see the full query (up to 30% of screen height)
- Shows "(+N more lines)" indicator when collapsed if query is longer
- Bordered box to visually separate from model responses
...
**3+ models with horizontal scroll (showing 2 columns):**

```
┌──────────────────────────────────────────────────────────────────────────────┐
│ Plan: 01HXYZ...  │  Query: 3/10  │  Models: 4  │  Showing: 1-2 of 4  ━━▶     │
├──────────────────────────────────────────────────────────────────────────────┤
│ Input: query_003.md                                                          │
│ What are the key differences between REST and GraphQL?                       │
├─────────────────────────────────────┬────────────────────────────────────────┤
...
│ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │ └────────────────────────────────────┘ │
├─────────────────────────────────────┴────────────────────────────────────────┤
│ h/l: focus  j/k: query  ↑↓/scroll: content  Tab: input  g/b: rate  ?: help   │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
### Tested (unit tests + non-interactive verification)
- [x] `tuna view <PlanID>` opens interactive viewer (command registered, loads data correctly)
- [x] Ratings persist to YAML front matter in response files (unit tested)
- [x] Front matter not rendered (stripped before display) (unit tested)
- [x] Ratings visible on re-opening viewer (unit tested)

### Implemented, requires manual testing (TUI needs TTY)
- [x] Columns fill all available screen width (no unused space)
- [x] Maximum 2 columns visible (1 model → 1 col, 2+ → 2)
- [x] Multiple model responses displayed side-by-side in columns
- [x] Focused column highlighted with distinct border style
- [x] Navigate between queries with j/k
- [x] Move focus between columns with ←/→
- [x] Horizontal scroll when more than 2 models
- [x] Scroll indicator shows visible range (e.g., "Showing: 1-2 of 4")
- [x] Each column independently scrollable (vertical)
- [x] Input query collapsible/expandable with Tab
- [x] Column heights adjust when input is expanded
- [x] Space/g/b keys update ratings for focused column
- [x] Clean exit with q/Esc
- [x] Help available with ?
- [x] Markdown responses render properly with glamour
