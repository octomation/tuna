...
- [ ] Run `go test ./internal/assistant/...` to verify unit tests pass
- [ ] Run `tuna init test-assistant` in a temp directory
- [ ] Verify directory structure is created correctly
- [ ] Run `tuna init test-assistant` again to verify idempotent behavior
- [ ] Test with invalid IDs: empty, `.`, `..`, `foo/bar`
- [ ] Test partial structure completion
