...
- [x] Configuration file is loaded from `.tuna.toml` or `~/.config/tuna.toml`
- [x] Multiple providers can be defined with their own credentials
- [x] Models are automatically routed to the correct provider
- [x] Default provider is used for unknown models
- [x] Backward compatibility with environment variables
- [x] `tuna config show/validate/resolve` commands work correctly
- [x] Rate limiting works per provider (configurable via `rate_limit`)
- [x] Model aliases resolve to full model names before provider lookup
- [x] Existing tests pass, new tests cover multi-provider scenarios
...
- [x] Configuration loads from `.tuna.toml`
- [x] Fallback to env variables works

**Phase 2**:
- [x] `go test ./internal/llm/...` passes
- [x] Router correctly routes requests
- [x] Rate limiting blocks on limit exceeded

**Phase 3**:
- [x] `go test ./...` passes (all existing tests)
- [x] `tuna exec` works with new configuration
- [x] `tuna config show/validate/resolve` work

**Phase 4**:
- [x] Documentation is up to date
- [x] Configuration example is correct
