...
- [x] `github.com/oklog/ulid/v2` added to dependencies
- [x] `github.com/google/uuid` removed from dependencies
- [x] `tuna plan` generates ULID-based plan IDs (26 characters)
- [x] Plans are sortable chronologically by folder name (`ls Output/` shows oldest first)
- [x] All existing tests pass with updated assertions
- [x] Documentation updated to reflect ULID usage
...
This will add `github.com/oklog/ulid/v2` and remove unused `github.com/google/uuid`.

### Step 2: Update `internal/plan/plan.go`
...
**Line 66:** Replace UUID generation with ULID
...
### Step 3: Update `internal/plan/plan_test.go`

**Lines 51-54:** Update ID format validation
