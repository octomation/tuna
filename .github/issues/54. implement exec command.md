...
### Core functionality
- [x] Plan is located by PlanID using glob pattern
- [x] Plan is parsed from `plan.toml` into a typed Go struct
- [x] `plan_id` in file is validated against PlanID argument
- [x] Query file content is read from `<AssistantID>/Input/`

### LLM integration
- [x] LLM requests use `github.com/sashabaranov/go-openai` library
- [x] Command reads `LLM_API_TOKEN` and `LLM_BASE_URL` from environment
- [x] Command makes a successful request using first model and first query
- [x] Response is printed to stdout

### Flags
- [x] `--dry-run` shows execution plan without API calls
- [x] `--parallel` prints "not implemented" warning
- [x] `--continue` prints "not implemented" warning

### Error handling
- [x] Clear error for missing plan file
- [x] Clear error for missing environment variables
- [x] Clear error for invalid TOML or mismatched plan_id
...
- [x] Run `go get github.com/sashabaranov/go-openai` to add dependency
- [x] Run `go build` to ensure no compilation errors
- [x] Run `go test ./internal/plan/...` to verify plan tests
- [x] Run `go test ./internal/llm/...` to verify LLM client tests
- [x] Run `go test ./internal/exec/...` to verify executor tests
- [x] Create test assistant and plan:
  ```bash
  tuna init test-assistant
  tuna plan test-assistant
  ```
- [x] Test dry-run mode: `tuna exec <plan-id> --dry-run`
- [x] Test with real API (requires LLM_API_TOKEN and LLM_BASE_URL):
  ```bash
  export LLM_API_TOKEN=your-token
  export LLM_BASE_URL=https://api.openai.com/v1
  tuna exec <plan-id>
  ```
- [x] Verify warning messages for --parallel and --continue flags
...
- [x] Run `go test ./internal/exec/...` for new tests
- [x] Test with dry-run: `tuna exec <plan-id> --dry-run`
...
- [x] Verify output files are created:
  ...
  ```
- [x] Verify summary shows correct totals
- [x] Test error handling: add invalid query ID to plan
